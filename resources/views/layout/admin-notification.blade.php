<div class="header-message-box ml-15 d-none d-md-flex">
    ...
    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="message">
        @forelse (Auth::user()->unreadNotifications->take(3) as $notification)
            <li>
                <a
                    href="{{ route('admin.readandredirect', ['id' => $notification->id, 'redirect' => base64_encode($notification->data[0]['adminurl'])]) }}">
                    <div class="image">
                        @isset($notification['data'][0]['newuser'])
                            <img src="{{ \App\Models\User::find($notification['data'][0]['newuser']['id'])->photo }}"
                                alt="{{ \App\Models\User::find($notification['data'][0]['newuser']['id'])->name }}">
                        @else
        ...
        <a href="{{ route('admin.notifications') }}" class="d-block text-center text-sm text-black">See All</a>
    </ul>
</div>
