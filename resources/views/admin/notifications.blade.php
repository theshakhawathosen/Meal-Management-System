                        ...
                        <div class="notification">
                            <div class="image primary-bg">
                                @isset($notification['data'][0]['newuser'])
                                    <img src="{{ \App\Models\User::find($notification['data'][0]['newuser']['id'])->photo }}" alt="{{ \App\Models\User::find($notification['data'][0]['newuser']['id'])->name}}">
                                @else
                                    <span>{{ Str::limit($notification['data'][0]['title'], 1, '') }}</span>
                                @endisset
                            </div>
                            <a href="{{ $notification['data'][0]['adminurl'] }}" class="content">
                        ...
                        <div class="action">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="moreAction">
                                <li class="dropdown-item">
                                    ...
                                    @else
                                        <a href="{{ route('admin.unreadnotification', $notification->id) }}"
        ...
        function deleteall() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "{{ route('admin.deleteallnotification') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                        },
                        success: function(response) {
